...
fn main() {
    ...
    std::assert!(
        (s140_v6 && !s140_v7) || (!s140_v6 && s140_v7),
        "need to have exactly one of nrf-s140-v6 or nrf-s140-v7 feature specified"
    );
    // Put the linker script somewhere the linker can find it
    let out_dir = &PathBuf::from(env::var_os("OUT_DIR").unwrap());
    let mut out_file = File::create(out_dir.join("memory.x")).unwrap();
    if cfg!(feature = "nrf-s140-v6") {
        out_file
            .write_all(include_bytes!("memory-s140_v6.x"))
            .unwrap();
    } else if cfg!(feature = "nrf-s140-v7") {
        out_file
            .write_all(include_bytes!("memory-s140_v7.x"))
            .unwrap();
    }
